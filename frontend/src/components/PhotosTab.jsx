...
import { Camera, ChevronDown, Trash2, FileDown, Loader2, AlertTriangle, Sparkles } from "lucide-react";
...
export const PhotosTab = ({ project }) => {
  ...
  if (!photos) return <p className="text-sm text-slate-400">Loading photos…</p>;

  const filtered = filter === "all" ? photos : photos.filter((p) => p.analysis.identified_stage === filter);
  const groups = PHOTO_STAGE_ORDER
    .map((stage) => ({ stage, items: filtered.filter((p) => p.analysis.identified_stage === stage) }))
  ...
  return (
    <div data-testid="photos-tab">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-2">
          <h3 className="font-heading text-lg font-bold uppercase tracking-wider text-slate-100">Photo Record</h3>
          <span className="text-xs text-slate-500" data-testid="photos-count">{photos.length} photo{photos.length === 1 ? "" : "s"}</span>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button data-testid="export-diary-button" variant="outline" onClick={() => setExportOpen(true)} disabled={photos.length === 0}
      ...
      {photos.length === 0 && (
        <div data-testid="photos-empty" className="rounded-md border border-slate-700 bg-slate-800/30 p-10 text-center">
          <p className="text-sm text-slate-400">No photos yet. Upload your first site photo above to start this project's visual build record.</p>
        </div>
      )}
      ...
      {groups.map((g) => (
        <section key={g.stage} className="mb-8" data-testid={`photo-group-${g.stage}`}>
          <div className="flex items-center gap-2 mb-3">
            <h4 className="text-xs uppercase tracking-[0.2em] text-amber-400 font-semibold">{stageLabel(g.stage)}</h4>
            <span className="text-xs text-slate-600">{g.items.length}</span>
            <div className="flex-1 h-px bg-slate-800" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 items-start">
            {g.items.map((p) => <PhotoCard key={p.id} photo={p} onDelete={deletePhoto} />)}
          </div>
        </section>
      ))}
